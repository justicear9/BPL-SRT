...
class SampleReportController extends Controller
{
    ...
    {
        $query = VisitSample::query()
            ->with(['visit.customer', 'visit.user', 'product']);
        ...
        $samples = $query->orderByDesc('id')->paginate(50)->withQueryString();

        $salesReps = $request->user()->canManageAllVisits()
            ? User::assignableFieldTeam()->get()
            : collect();

        return view('content.reports.sample-report', [
            'samples' => $samples,
            'salesReps' => $salesReps,
        ]);
    }
