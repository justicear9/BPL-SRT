...
function loadSearchData() {
  const customUrl = document.documentElement.getAttribute('data-navigation-search-url');
  const searchJson = $('#layout-menu').hasClass('menu-horizontal') ? 'search-horizontal.json' : 'search-vertical.json';
  const url = customUrl && customUrl.length ? customUrl : assetsPath + 'json/' + searchJson;
...
function initializeAutocomplete() {
  ...
  return autocomplete({
    ...
    getSources() {
      ...
      if (data.customers && Array.isArray(data.customers) && data.customers.length) {
        sources.push({
          sourceId: 'customers',
          getItems({ query }) {
          ...
          templates: {
            header({ items, html }) {
              if (items.length === 0) return null;
              return html`<span class="search-headings">Customers</span>`;
            },
            item({ item, html }) {
              return html`
                <a href="${baseUrl}${item.url}" class="d-flex justify-content-between align-items-center">
                  <span class="item-wrapper d-flex align-items-start gap-2">
                    <i class="icon-base ti ${item.icon || 'tabler-building-store'} mt-1"></i>
                    <span>
                      <span class="d-block">${item.name}</span>
                      ${item.subtitle ? html`<small class="text-body-secondary d-block">${item.subtitle}</small>` : null}
                    </span>
                  </span>
                  <svg xmlns="http://www.w3.org/2000/svg" width="20px" height="20px" viewBox="0 0 24 24">
                    <g
...
document.addEventListener('keydown', event => {
  if ((event.ctrlKey || event.metaKey) && event.key === 'k') {
    event.preventDefault();
    const detached = document.querySelector('.aa-DetachedSearchButton');
    if (detached) {
      detached.click();
      return;
    }
